...
export async function run() {
    try {
        await terraformCommandHandlerGCP.refresh();
    } catch(error) {
        tl.setResult(tl.TaskResult.Failed, 'GCPRefreshFailInvalidWorkingDirectoryL0 should have succeeded but failed with error.');
    }
}
