...
enum EnvironmentVariables {
    HTTP_PROXY = <any>"HTTP_PROXY", 
    HTTPS_PROXY = <any>"HTTPS_PROXY",
}
    ...
    private _requestRaw(info: RequestInfo, data: string | NodeJS.ReadableStream): Promise<HttpClientResponse> {
        return new Promise<HttpClientResponse>((resolve, reject) => {
            ...
            if (typeof (data) === 'string') { 
                info.options.headers["Content-Length"] = Buffer.byteLength(<string>data, 'utf8');
            }
            ...
            if (data && typeof (data) !== 'string') {
                (<NodeJS.ReadableStream>data).on('close', function () {
                    req.end();
                });

                (<NodeJS.ReadableStream>data).pipe(req);
            }
            else {
                req.end();
            }
