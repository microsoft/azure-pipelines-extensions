...
export default class ExperimentManager {
    ...
    public async executeAction(action: ExperimentAction, experiment: any): Promise<void> {
        ...
        switch(action) {
            case ExperimentAction.Start: { 
                if (this._isRestartRequired(experiment)) {
    ...
    */
    ...
    private _isRestartRequired(experiment: any): boolean {
        // restart the experiment if it is already stopped
        // an experiment is considered stopped if any of its stages is stopped (StopTime has been set)
