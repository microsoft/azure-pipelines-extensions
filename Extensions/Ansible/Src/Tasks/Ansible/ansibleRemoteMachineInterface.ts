...
export class ansibleRemoteMachineInterface extends ansibleCommandLineInterface {
  ...
  protected async executeCommand(cmd: string): Promise<string> {
    return await ansibleUtils.runCommandOnRemoteMachine(
      cmd,
      this._sshClientConnection,
      this._remoteCmdOptions
    );
  }
  ...
  protected async setupConnection() {
    ...
    var username: string = tl.getEndpointAuthorizationParameter(
      sshEndpoint,
      "username",
      false
    );
    var password: string = tl.getEndpointAuthorizationParameter(
      sshEndpoint,
      "password",
      true
    ...
    var hostname: string = tl.getEndpointDataParameter(
      sshEndpoint,
      "host",
      false
    );
    ...
    ansibleUtils._writeLine(
      tl.loc(
        "SettingUpSshConnection",
    ...
    try {
      this._sshClientConnection = await ansibleUtils.setupSshClientConnection(
        this._sshConfig
      );
    } catch (err) {
