...
export class ansibleTowerInterface extends ansibleInterface {
  ...
  public async execute() {
    try {
      ...
      if (status === "successful") tl.setResult(tl.TaskResult.Succeeded, "");
      else if (status === "failed") tl.setResult(tl.TaskResult.Failed, "");
    } catch (error) {
  ...
  private initializeTaskContants() {
    try {
      ...
      tl.setResult(
        ...
        tl.loc("Ansible_ConstructorFailed", error.message)
      );
    }
  }
  ...
  private getJobLaunchApi(): string {
    var ansibleJobLaunchUrl: string = util.format(
      this._jobLaunchUrlFormat,
      this._hostname,
      this._jobTemplateId
    );
  ...
  private getJobApi(jobId: string): string {
    var ansibleJobUrl: string = util.format(
      this._jobUrlFormat,
      this._hostname,
      jobId
    );
  ...
  private getJobEventApi(
    jobId: string,
    pageSize: number,
    pageNumber: number
  ): string {
    var ansibleJobEventUrl: string = util.format(
      this._jobEventUrlFormat,
      this._hostname,
      jobId,
      pageSize,
      pageNumber
  ...
  private async getJobTemplateId(): Promise<string> {
    ...
    request.method = "GET";
    request.uri = util.format(
      this._jobTemplateIdUrlFormat,
      this._hostname,
      this._jobTemplateName
    );
  ...
  private async getJobEvents(
    jobId: string,
    lastDisplayedEvent: number
  ): Promise<string[]> {
  ...
  private _jobEventUrlFormat: string =
    "%s/api/v1/jobs/%s/job_events/?page_size=%s&page=%s";
  private _jobTemplateIdUrlFormat: string =
    "%s/api/v1/job_templates/?name__exact=%s";
