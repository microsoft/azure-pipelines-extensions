...
import { DataSourceInfo,PayloadForExecuteServiceEndpointRequest,PayloadForUpdateDataSource, PayloadForUpdateDataSourceParameters, PayloadForSelectDataSource, ParamInfo } from '../states/DataSourceExtensionState';
            ...
            serviceEndpointRestClient.getServiceEndpointDetails(project, endpointId).then((endpointdetails: ServiceEndpointDetails) => {
                serviceEndpointRestClient.getServiceEndpointTypes(endpointdetails.type).then((typeDetails: ServiceEndpointType[]) => {
                    if (typeDetails.length == 1) {
                        ...
                        DataSourceActions.GetDataSources(data,endpointdetails);    
                    }
                    else{
                        DataSourceActions.GetDataSources(undefined,endpointdetails);
                    }
                });
            });
        }  
