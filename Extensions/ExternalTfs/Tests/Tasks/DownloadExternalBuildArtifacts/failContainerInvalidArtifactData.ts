...
// Container data with only two segments instead of the expected three.
// Triggers `if (containerParts.length !== 3) throw new Error(tl.loc("FileContainerInvalidArtifactData"))`.
registerAllMocks(tr, {
    artifacts: [{ type: 'container', name: 'drop', data: '#/123' }]
});
