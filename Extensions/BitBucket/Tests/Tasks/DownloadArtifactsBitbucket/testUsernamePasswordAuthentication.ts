...
// Set task inputs
runner.setInput('connection', 'bitbucket-userpass-endpoint');
...
process.env["ENDPOINT_AUTH_SCHEME_bitbucket-userpass-endpoint"] = "UsernamePassword";
process.env["ENDPOINT_AUTH_PARAMETER_bitbucket-userpass-endpoint_USERNAME"] = "testuser";
process.env["ENDPOINT_AUTH_PARAMETER_bitbucket-userpass-endpoint_PASSWORD"] = "testpassword";
process.env["ENDPOINT_AUTH_bitbucket-userpass-endpoint"] = JSON.stringify({
...
runner.registerMock('https', {
    request: function(options: any, callback: Function) {
        ...
        console.log('Using username/password auth: testuser');
        var mockResponse = {
            statusCode: 200,
            statusMessage: 'OK',
...
runner.registerMock('./sourcecontrolwrapper.js', {
    SourceControlWrapper: function(toolType: string) {
        ...
        this.clone = function(repo: string, progress: boolean, folder: string, options: any) {
            console.log('git clone called: ' + repo + ' -> ' + folder);
            var defer = Q.defer();
            defer.resolve(0);
