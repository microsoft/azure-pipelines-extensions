...
// Set task inputs - using refs/heads/ prefix
runner.setInput('connection', 'bitbucket-token-endpoint');
runner.setInput('definition', 'testuser/testrepo');
runner.setInput('branch', 'refs/heads/main');  // Branch with refs/heads/ prefix
...
runner.registerMock('./sourcecontrolwrapper.js', {
    SourceControlWrapper: function(toolType: string) {
        ...
        this.checkout = function(ref: string, options?: any) {
            ...
            // Verify refs/heads/ was converted to refs/remotes/origin/
            if (ref === 'refs/remotes/origin/main') {
                console.log('SUCCESS: refs/heads/ prefix correctly converted to refs/remotes/origin/');
            }
            var defer = Q.defer();
            defer.resolve(0);
            return defer.promise;
