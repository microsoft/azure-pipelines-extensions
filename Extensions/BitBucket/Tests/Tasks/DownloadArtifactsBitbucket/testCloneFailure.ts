...
// Mock sourcecontrolwrapper - simulate clone failure
runner.registerMock('./sourcecontrolwrapper.js', {
    SourceControlWrapper: function(toolType: string) {
        ...
        this.clone = function(repo: string, progress: boolean, folder: string, options: any) {
            console.log('git clone called - simulating failure');
            var defer = Q.defer();
            defer.reject(new Error('Clone failed: repository access denied'));
