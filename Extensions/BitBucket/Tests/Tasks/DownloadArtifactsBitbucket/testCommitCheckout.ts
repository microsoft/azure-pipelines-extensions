...
// Set task inputs with specific commit ID
const specificCommitId = 'abc123def456';
...
runner.setInput('version', specificCommitId);
...
// Track checkout calls to verify order
...
runner.registerMock('./sourcecontrolwrapper.js', {
    SourceControlWrapper: function(toolType: string) {
        ...
        this.checkout = function(ref: string, options?: any) {
            checkoutCalls.push(ref);
            console.log('git checkout called: ' + ref);
            // Verify that commit ID checkout happens after branch checkout
            if (ref === specificCommitId && checkoutCalls.length === 2) {
                console.log('SUCCESS: Commit checkout correctly called after branch checkout');
            }
            var defer = Q.defer();
            defer.resolve(0);
            return defer.promise;
