...
// Mock https - verify API v2.0 path
runner.registerMock('https', {
    request: function(options: any, callback: Function) {
        ...
        if (options.path.includes('/2.0/repositories/')) {
            console.log('SUCCESS: API v2.0 endpoint used correctly');
        } else if (options.path.includes('/1.0/')) {
            console.log('ERROR: Deprecated API v1.0 endpoint used!');
        }
        var mockResponse = {
            statusCode: 200,
            statusMessage: 'OK',
