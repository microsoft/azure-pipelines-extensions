...
// Set environment variables for endpoint - need BOTH individual params AND the full JSON
...
// This is the key one - getEndpointAuthorization reads this JSON
...
runner.registerMock('shelljs', {
    rm: function(flags: string, path: string) {
...
// Mock https - return successful response with git repo
runner.registerMock('https', {
    request: function(options: any, callback: Function) {
        ...
        console.log('Auth header present: ' + (options.headers && options.headers['Authorization'] ? 'yes' : 'no'));
...
// Mock sourcecontrolwrapper - use Q promises
